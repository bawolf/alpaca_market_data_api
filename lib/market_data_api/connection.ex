...
defmodule MarketDataAPI.Connection do
  @moduledoc """
  Handle Tesla connections for MarketDataAPI.

  Additional middleware can be set in the compile-time or runtime configuration:

      config :tesla, MarketDataAPI.Connection,
  ...
  def middleware(options \\ []) do
    base_url =
      Keyword.get(
        options,
        :base_url,
        Application.get_env(:alpaca_market_data_api, :base_url, @default_base_url)
      )
    ...
    user_agent =
      Keyword.get(
        ...
        Keyword.get(
          tesla_options,
          :user_agent,
          "openapi-generator - MarketDataAPI 2.0.0 - elixir"
        )
      )
    ...
    api_client_id =
      Keyword.get(
        options,
        :username,
        Application.get_env(:alpaca_market_data_api, :api_client_id)
      )
    ...
    api_secret =
      Keyword.get(
        options,
        :password,
        Application.get_env(:alpaca_market_data_api, :api_secret)
      )

    headers = [{"APCA-API-KEY-ID", api_client_id}, {"APCA-API-SECRET-KEY", api_secret}]

    [
      {Tesla.Middleware.BaseUrl, base_url},
      {Tesla.Middleware.Headers, [{"user-agent", user_agent} | headers]},
